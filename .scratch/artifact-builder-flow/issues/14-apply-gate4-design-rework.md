...
3. **Theme library created** — `artifact-builder/docs/design/themes.md`: festival-dark (ADR-001 tokens, demoted) + paper-editorial + terminal + bold-signal, each a token block + one-line voice + when-to-use; selection rule = subject-first, never default unasked.
