...
- [Grilling: responsive + touch baseline](issues/05-grilling-responsive-touch-baseline.md) — locked + implemented: ADR-001 "Device support baseline" section (platform contract, all presets): viewport-fit=cover + env() safe-area pads + ≥16px inputs, no user-scalable=no; tap targets 44px/40px-floor/8px-gaps, no hover-only, active+focus-visible; single-col ≤640 / 2-col ≥960, recharts min-height 220/300, h-overflow at 375px = FAIL; sim 60fps + hidden-pause; gate-6 mobile/throttled pass (375×667, CPU 4x).
- [Prototype: gallery index page](issues/09-prototype-gallery-index-page.md) — built + live at https://azizmaitig.github.io/artifact-gallery/: card list (accent tile, name, desc, Android/iOS install copy), ADR-001 token style, static JSON artifact array for maintenance, root-scoped sw.js (06 pattern). Verified 375×667 (no overflow, console-clean incl. data: favicon fix); Pages cache convergence applies.
